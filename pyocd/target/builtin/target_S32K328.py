...
class S32K328(S32K3XX):

    VENDOR = "NXP"

    MEMORY_MAP = MemoryMap(
        FlashRegion(name="pflash",  start=0x00400000, end=0xbfffff, blocksize=0x2000, page_size=FLASH_ALGO.get('page_size'), is_boot_memory=True, algo=FLASH_ALGO),
        ...
        RamRegion(name="sram",      start=0x20400000, length=0xC0000), # 768 KB
        )

    def __init__(self, session):
        super(S32K328, self).__init__(session, self.MEMORY_MAP)
    ...
    def reset(self, reset_type=None):
        super(S32K328, self).reset(self.ResetType.SW_VECTRESET)

    def reset_and_halt(self, reset_type=None, map_to_user=True):
        super(S32K328, self).reset_and_halt(self.ResetType.SW_VECTRESET)

    def create_init_sequence(self):
        seq = super(S32K328, self).create_init_sequence()
